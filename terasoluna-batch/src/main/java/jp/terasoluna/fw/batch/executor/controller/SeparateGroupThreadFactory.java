 ...
 * 業務コンテキストの一意識別キーとしてスレッドグループを使用するため、
 * 本クラスを併用せずに{@code ThreadGroupApplicationContextHolder}を使用した場合、
 * 意図しない動作となる。
 * <p>
 * スレッドグループとスレッド名のプリフィックスの内側に付与される番号は本クラスで管理するが、
 * スレッド名の末尾に付与される番号は{@code CustomizableThreadCreator}で管理される。
 * <p>
 * 本クラスはAPIとして、以下使用時の前提を持つ。
 * <ul>
 * <li>スレッドの新規生成処理はシングルスレッドで実行される。</li>
 * <li>スレッドの新規生成回数は有限である。</li>
 * </ul>
 * この前提に沿わない場合、以下の動作となる点に注意すること。
 * <ol>
 * <li>本クラスによるスレッドの新規生成を複数スレッドから行った場合、同名のスレッドが生成される恐れがある。</li>
 * <li>スレッドグループは新規スレッド生成毎に生成され、共有・破棄は行われないためメモリ上に蓄積する。</li>
 * </ol>
 * なお、本クラスのインジェクション対象である{@code ThreadPoolTaskExecutor}による
