...
public class GenericsUtil {
     ...
     * <p>
     * <h5>実際の型の取得の可否</h5> このクラスで実際の型を取得できるのは、クラス宣言で実際の型が 指定されている場合である。 クラス宣言で実際の型が指定されていない場合、 クラス宣言で<code>WildCardType</code>が
     * 指定されている場合、および、コード中で変数宣言の際に実際の型が 指定されている場合は取得できない。
     * <ul>
     * <li>取得できる例(子クラスので具体クラスが指定されている) <code><pre>
     *     public class Descendant extends Generic&lt;String, Integer&gt; {
     *        ...
     *     }
     *     </pre></code> この場合、正しく[<code>String</code>, <code>Integer</code>]の 配列が取得できる</li>
     * <li>取得できる例(子クラスのクラス宣言で配列が指定されている) <code><pre>
     *     public class Descendant extends Generic&lt;String[], Integer&gt; {
     *        ...
     *     }
     *     </pre></code> この場合、正しく[<code>String[]</code>, <code>Integer</code>]の 配列が取得できる</li>
     * <li>取得できる例(子クラスのクラス宣言で型パラメータを持つ具体クラスが指定されている) <code><pre>
     *     public class Descendant
     *         extends Generic&lt;String[], Map&lt;String, Object&gt;&gt; {
     *        ...
     *     }
     *     </pre></code> この場合、正しく[<code>String[]</code>, <code>Map</code>]の 配列が取得できる</li>
     * <li>取得できない例(子クラスのクラス宣言で型パラメータが指定されている) <code><pre>
     *     public class Descendant&lt;P, Q&gt; extends Generic&lt;S, T&gt; {
     *        ...
     *     }
     *     </pre></code></li>
     * <li>取得できない例(子クラスのクラス宣言でワイルドカードが指定されている) <code><pre>
     *     public class Descendant&lt;P extends String, Q super Bean&gt;
     *         extends Generic&lt;S, T&gt; {
     *        ...
     *     }
     *     </pre></code></li>
     * <li>取得できない例(コード中で具体クラスが指定されている) <code><pre>
     ...
     * </p>
     * <p>
     * <h5>多世代継承の場合</h5> <code>genericType</code>から<code>descendantClass</code>まで 多世代の継承がある場合も、実際の型を取得することができる。
     * <ul>
     * <li>多世代継承の例 <code><pre>
     ...
     * </p>
     * <p>
     * <h5>型パラメータの順番が変更された場合</h5> <code>genericType</code>から<code>descendantClass</code>までの 継承の過程で型パラメータの順番が入れ替わった場合でも、
     * <code>genercClass</code>で宣言された順番で実際の型を取得できる。
     * <ul>
     * <li>順番が入れ替わる場合の例 <code><pre>
     ...
     * @param descendantClass <code>genericsClass</code>を継承し、 具体的な型パラメータを指定したクラス。
     * @return 実際の型を表す<code>Class</code>インスタンスの配列。 順番は<code>genercClass</code>で宣言された順番である。
     * @throws IllegalArgumentException 引数<code>genericClass</code>が <code>null</code>の場合。 引数<code>descendantClass</code>が
     *             <code>null</code>の場合。
     * @throws IllegalStateException <code>descendantClass</code>の実装で型パラメータが 具体クラスとして指定されていない場合。 引数<code>genercClass</code>
     *             が型パラメータを宣言したクラスでは なかった場合。
     */
    ...
    public static <T> Class[] resolveParameterizedClass(Class<T> genericClass,
        ...
        if (genericClass == null) {
            throw new IllegalArgumentException("Argument 'genericsClass' ("
                    + Class.class.getName() + ") is null");
        }
        if (descendantClass == null) {
            throw new IllegalArgumentException("Argument 'descendantClass'("
                    + Class.class.getName() + ") is null");
        }

        List<ParameterizedType> ancestorTypeList = getAncestorTypeList(
                genericClass, descendantClass);

        ParameterizedType parameterizedType = ancestorTypeList.get(
                ancestorTypeList.size() - 1);
        ...
        for (int i = 0; i < actualTypes.length; i++) {
            ...
            actualClasses[i] = resolveTypeVariable(actualTypes[i],
                    ancestorTypeList);
        }
     ...
     *        ...
     *     }
     *     </pre></code> この場合、正しく<code>String</code>または<code>Integer</code>が 取得できる</li>
     * <li>取得できる例(子クラスのクラス宣言で配列が指定されている) <code><pre>
     *     public class Descendant extends Generic&lt;String[], Integer&gt; {
     *        ...
     *     }
     ...
     *        ...
     *     }
     *     </pre></code> この場合、正しく<code>String[]</code>または<code>Map</code> が取得できる</li>
     * <li>取得できない例(子クラスのクラス宣言で型パラメータが指定されている) <code><pre>
     *     public class Descendant&lt;P, Q&gt; extends Generic&lt;S, T&gt; {
     *        ...
     *     }
     ...
     *          ...
     *       }
     * </pre></code> この場合、正しく<code>String</code>または<code>Integer</code> が取得できる</li>
     * </ul>
     * </p>
     * <p>
     * <h5>型パラメータの順番が変更された場合</h5> <code>genericType</code>から<code>descendantClass</code>までの 継承の過程で型パラメータの順番が入れ替わった場合でも、
     ...
     * @throws IllegalArgumentException 引数<code>genericClass</code>が <code>null</code>の場合。 引数<code>descendantClass</code>が
     *             <code>null</code>の場合。 引数<code>index</code>が<code>0</code>より小さい、または、 宣言された型パラメータ数以上の場合。
     * @throws IllegalStateException <code>descendantClass</code>の実装で型パラメータが 具体クラスとして指定されていない場合。 引数<code>genercClass</code>
     *             が型パラメータを宣言したクラスでは なかった場合。
     */
    public static <T> Class<? extends Object> resolveParameterizedClass(
        ...
        if (index < 0 || index >= actualTypes.length) {
            throw new IllegalArgumentException("Argument 'index'(" + Integer
                    .toString(index) + ") is out of bounds of"
                    + " generics parameters");
        }
    ...
    /**
     * 特定の型から型パラメータを宣言したクラスまでの <code>ParameterizedType</code>のリストを取得する。
     * @param <T> 型パラメータ宣言をしたクラスの型。
     * @param genericClass 型パラメータ宣言をしたクラス。
     * @param descendantClass <code>genericsClass</code>を継承し、 具体的な型パラメータを指定したクラス。
     * @return 特定の型から型パラメータを宣言したクラスまでの <code>ParameterizedType</code>のリスト。
     * @throws IllegalStateException <code>descendantClass</code>の 実装で型パラメータが具体クラスとして指定されていない場合。 引数<code>genercClass</code>
     *             が型パラメータを宣言したクラスでは なかった場合。
     */
    protected static <T> List<ParameterizedType> getAncestorTypeList(
        ...
        while (clazz != null) {
            ...
            if (checkInterfaceAncestors(genericClass, ancestorTypeList,
                    clazz)) {
                break;
            }
        ...
        if (ancestorTypeList.isEmpty()) {
            throw new IllegalStateException("Argument 'genericClass'("
        ...
        // 理由は、GenericsのリフレクションAPIについては実装が不定であるためである。
        // 問題がある場合は、削除すること。
        ParameterizedType targetType = ancestorTypeList.get(ancestorTypeList
                .size() - 1);
        if (!targetType.getRawType().equals(genericClass)) {
            throw new IllegalStateException("Class(" + descendantClass.getName()
                    + ") is not concrete class of Class(" + genericClass
                            .getName() + ")");
        }
    ...
    /**
     * インタフェース型から型パラメータを宣言したクラスまでの <code>ParameterizedType</code>のリストを取得する。
     * @param <T> 型パラメータを宣言したクラスの型。
     * @param genericClass 型パラメータを宣言したクラス。
     * @param ancestorTypeList <code>ParameterizedType</code>を 追加するリスト。
     * @param clazz 検査対象のインタフェース型。
     * @return 型パラメータを宣言したクラスが見つかった場合は<code>true</code>。 見つからなかった場合は<code>false</code>。
     */
    protected static <T> boolean checkInterfaceAncestors(Class<T> genericClass,
        ...
        for (Type interfaceType : interfaceTypes) {
            genericTypeFound = checkParameterizedType(interfaceType,
                    genericClass, ancestorTypeList);
            if (genericTypeFound) {
                return true;
            }
            @SuppressWarnings("rawtypes")
    ...
    /**
     * <code>Type</code>型をチェックし、<code>ParameterizedType</code> かつ、型パラメータを宣言したクラスのサブクラスである場合、リストに追加する。
     * @param <T> 型パラメータを宣言したクラスの型。
     * @param type 検査対象の型。
     * @param genericClass 型パラメータを宣言したクラス。
     * @param ancestorTypeList <code>ParameterizedType</code>を 追加するリスト。
    ...
    protected static <T> boolean checkParameterizedType(Type type,
            Class<T> genericClass, List<ParameterizedType> ancestorTypeList) {
        ...
        // 理由は、GenericsのリフレクションAPIについては実装が不定であるためである。
        // 問題がある場合は、削除すること。
        if (!genericClass.isAssignableFrom((Class<?>) parameterlizedType
                .getRawType())) {
            return false;
        }
     ...
     * @param ancestorTypeList <code>type</code>の具体的な型が 宣言されている可能性のある<code>ParameterizedType</code>のリスト。
     * @return 実行時の型変数。
     * @throws IllegalStateException 引数<code>type</code>が <code>Class</code>型、および、 <code>TypeVariable</code>型ではない場合。 引数
    ...
    protected static Class<? extends Object> resolveTypeVariable(Type type,
        ...
        for (int i = ancestorTypeList.size() - 1; i >= 0; i--) {
            ...
            if (!(declaration instanceof Class)) {
                throw new IllegalStateException("TypeVariable(" + targetType
                        .getName() + " is not declared at Class "
            ...
            if (index == -1) {
                ...
                // 理由は、GenericsのリフレクションAPIについては実装が不定であるためである。
                // 問題がある場合は、削除すること。
                throw new IllegalStateException("Class(" + declaredClass
                        .getName() + ") does not declare TypeValidable("
                        + targetType.getName() + ")");
            }
        ...
        throw new IllegalStateException("Concrete type of Type(" + type
                + ") was not found in ancestorList(" + ancestorTypeList + ")");
    }

    /**
     * 引数<code>type</code>が<code>Class</code>型 であるか、<code>TypeVariable</code>型かを判定する。
     * @param type <code>Type</code>インスタンス。
     * @return 引数<code>type</code>が <code>Class, ParameterizedType, GenericArrayType</code>の場合 <code>true</code>。 引数
     *         <code>type</code>が<code>TypeVariable</code>の場合 <code>false</code>。
     * @throws IllegalStateException 引数<code>type</code>が <code>Class</code>、<code>ParameterizedType</code>、
     *             <code>GenericArrayType</code>、<code>TypeVariable</code>の いずれでもない場合。
     */
    protected static boolean isNotTypeVariable(
        ...
        throw new IllegalStateException("Type(" + type + ") is not instance of "
                + TypeVariable.class.getName() + ", " + ParameterizedType.class
                        .getName() + ", " + GenericArrayType.class.getName()
                + " nor " + Class.class.getName());
    }
    ...
    protected static Class<? extends Object> getRawClass(
            Type type) throws IllegalStateException {
        if (type instanceof Class) {
