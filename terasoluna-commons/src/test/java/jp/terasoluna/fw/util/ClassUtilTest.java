...
public class ClassUtilTest {
     ...
     * 期待値 :生成したインスタンス<br>
     * 生成するオブジェクトのTerasolunaに対応したクラス名を元に インスタンスを生成することを確認する。<br>
     */
    @Test
    public void testCreateString02() {
        ...
        try {
            ...
            // 結果確認
            assertEquals("jp.terasoluna.fw.util.ClassUtil", obj.getClass()
                    .getName());
        } catch (ClassLoadException e) {
            fail();
        }
     ...
     * 指定したクラスがnullコンストラクタを保持しないクラスの場合、 InstantiationExceptionが発生することを確認する。<br>
     */
    @Test
     ...
     * 指定したクラスが存在しない場合、 ClassNotFoundExceptionが発生することを確認する。<br>
     */
    @Test
     ...
     * 指定したクラスのコンストラクタにアクセスできない場合、 IllegalAccessExceptionが発生することを確認する。<br>
     */
    @Test
     ...
     * 期待値 :生成したインスタンス<br>
     * コンストラクタの引数が1つの時、 生成するオブジェクトのクラス名を元にインスタンスを生成することを確認する。<br>
     */
    @Test
     ...
     * 期待値 :生成したインスタンス<br>
     * コンストラクタの引数が空の時、 生成するオブジェクトのクラス名を元にインスタンスを生成することを確認する。<br>
     */
    @Test
     ...
     * 期待値 :生成したインスタンス<br>
     * コンストラクタの引数が2つの時、 生成するオブジェクトのクラス名を元にインスタンスを生成することを確認する。<br>
     */
    @Test
    public void testCreateStringObjectArray03() {
        ...
        try {
            // テスト実行
            Object obj = ClassUtil.create(input, new Object[] { language,
                    country });
     ...
     * 入力値 :*.classがないクラス名<br>
     * 期待値 :ClassNotFoundException<br>
     * 指定したクラスが存在しない場合、ClassNotFoundExceptionが発生し、 それをラップするClassLoadExceptionが発生することを確認する。<br>
     */
    @Test
     ...
     * 入力値 :抽象クラス<br>
     * 期待値 :InstantiationException<br>
     * 指定したクラスが抽象クラスの場合、InstantiationExceptionが発生し、 それをラップする例外ClassLoadExceptionが発生することを確認する。<br>
     */
    @Test
     ...
     * オブジェクトが生成できなかった場合、InvocationTargetExceptionが発生し、 それをラップする例外ClassLoadExceptionが発生することを確認する。<br>
     */
    @Test
     ...
     * オブジェクトが生成できなかった場合、IllegalArgumentExceptionが発生し、 それをラップする例外ClassLoadExceptionが発生することを確認する。<br>
     */
    @Test
    public void testCreateStringObjectArray08() {
        ...
        try {
            ...
            if (e.getCause().getMessage().indexOf(
                    "class name is java.lang.Void") == -1) {
                // 結果確認
                fail();
            }
        }
