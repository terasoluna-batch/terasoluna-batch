 ...
 * <p>
 * 可変長ファイル用のFileLineWriterを生成する。
 * </p>
 * 行オブジェクトに設定出来るアノテーションの説明は{@link VariableFileLineWriter} のJavaDocを参考して下さい。
 */
public class VariableFileUpdateDAO extends AbstractFileUpdateDAO {
     ...
     * @return 可変長ファイル用のFileLineWriter
     */
    @Override
    public <T> FileLineWriter<T> execute(String fileName, Class<T> clazz) {

        // FileLineWriterを生成する。
        VariableFileLineWriter<T> fileLineWriter = new VariableFileLineWriter<T>(
